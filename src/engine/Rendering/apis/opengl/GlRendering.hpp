...
{
public:
	GlImage(const char *filePath, ImageSettings settings);
	GlImage(GLuint id, glm::vec2 size);
	GlImage(SDL_Surface *surface, ImageSettings settings);
	~GlImage();
...
private:
	void setupObject(SDL_Surface *surface, ImageSettings settings);
	GLuint textureID = 0;
	GLuint samplerID = 0;
};
